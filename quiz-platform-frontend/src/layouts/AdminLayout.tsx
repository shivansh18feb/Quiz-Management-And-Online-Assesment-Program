...
export const AdminLayout: React.FC = () => {
  ...
  const navLinks = [
    ...
    { path: ROUTES.ADMIN.QUIZZES, label: 'Quizzes', icon: BookOpen },
    { path: ROUTES.ADMIN.CATEGORIES, label: 'Categories', icon: FolderTree },
    { path: ROUTES.ADMIN.ATTEMPTS, label: 'Attempts', icon: History },
    { path: ROUTES.ADMIN.LEADERBOARD, label: 'Leaderboard', icon: Trophy },
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          {navLinks.map((link) => (
            <NavLink
              ...
              end={link.path === ROUTES.ADMIN.DASHBOARD}
            >
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5">
              ...
              <div className="flex flex-col hidden sm:flex">
                ...
                <span className="text-xs text-primary-400 mt-1 leading-none">Admin</span>
              </div>
            </div>
          </div>
        </header>
